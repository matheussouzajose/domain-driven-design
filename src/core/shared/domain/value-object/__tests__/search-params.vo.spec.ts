...
describe('Search Params Unit Tests', () => {
  ...
  ])('per_page prop %p', ({ per_page, expected }) => {
    const params = new SearchParams();
  ...
  test.each([
    ...
    { sort_dir: 'DESC', expected: 'desc' },
  ])('sort_dir prop %p', ({ sort_dir, expected }) => {
    ...
    expect(
      new SearchParams({ sort: 'field', sort_dir: sort_dir as any }).sort_dir,
    ).toBe(expected);
  });
  ...
  ])('filter props %p', ({ filter, expected }) => {
    const params = new SearchParams();
